...
def update_n(old_df: pandas.DataFrame, new_df: pandas.DataFrame) -> pandas.DataFrame:
    ...
    """
    assert list(old_df.columns) == ['anon-ip', 'game', 'n'], f"Columns  of doc. DataFrame must be {['anon-ip', 'game', 'n']} but were {old_df.columns}"
    assert list(new_df.columns) == ['anon-ip', 'game', 'n'], f"Columns  of new DataFrame must be {['anon-ip', 'game', 'n']} but were {new_df.columns}"

    # Replace NaN values with zero.
    old_df = old_df.merge(new_df, how="outer", on=['anon-ip', 'game'], suffixes=['_documented', '_measured']).fillna(0)
...
def standardize_to_lower_case_game(df: pandas.DataFrame) -> pandas.DataFrame:
    assert list(df.columns) == ['date','anon-ip', 'game', 'lang'], f"Columns  of DataFrame must be {['date','anon-ip', 'game', 'lang']} but were {df.columns}"
...
def filter_home_page_accesses(df: pandas.DataFrame) -> pandas.DataFrame:
    ...
    unique['count'] = unique.groupby(['date', 'anon-ip', 'lang']).transform('count')
    ...
    if len(home_page_access) == 5 and len(list(potential_home_page_games - home_page_games)) == 0:
        ...
        outer_join = df.merge(home_page_access, how='outer', on=['date', 'anon-ip', 'game', 'lang'], indicator=True)
        # 2. Disqualify every row with indicator "both" from the outer join and remove columns added in previous step.
        anti_join = outer_join[~(outer_join._merge == 'both')].drop(['_merge', 'count', 'index'], axis = 1).reset_index()
        return anti_join
...
def measure_access(doc_measurements_path: str, new_measurements_path) -> pandas.DataFrame:
    ...
    new_df = pandas.read_csv(new_measurements_path, delimiter=";", index_col=False)
    ...
    doc_df = pandas.read_csv(doc_measurements_path, delimiter=";", index_col=False)
