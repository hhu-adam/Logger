...
class TestMeasurement(unittest.TestCase):
    ...
    def test_measuring_access_with_empty_documented_measurements(self):
        exp_df = pandas.read_csv(self.expected_initialized_documentation, delimiter=';', index_col=False)
        res_df = measure_access(self.empty_documented_measurements, self.new_measurement)
    ...
    def test_measuring_access_with_empty_documented_measurements_with_home_page_access_non_shuffled(self):
        exp_df = pandas.read_csv(self.expected_initialized_documentation_without_home_page_access, delimiter=';', index_col=False)
        res_df = measure_access(self.empty_documented_measurements, self.new_measurement_with_home_page_access_non_shuffled)
        self.assertTrue(exp_df.equals(res_df))

    
    def test_measuring_access_with_empty_documented_measurements_with_home_page_access_shuffled(self):
        exp_df = pandas.read_csv(self.expected_initialized_documentation_without_home_page_access, delimiter=';', index_col=False)
        res_df = measure_access(self.empty_documented_measurements, self.new_measurement_with_home_page_access_shuffled)
        self.assertTrue(exp_df.equals(res_df))


    def test_updating_already_existing_documentation(self):
        exp_df = pandas.read_csv(self.expected_updated_documentation, delimiter=';', index_col=False)
        res_df = measure_access(self.documented_measurements, self.new_measurement)
        self.assertTrue(exp_df.equals(res_df))


    def test_updating_already_existing_documentation_with_home_page_access_shuffled(self):
        exp_df = pandas.read_csv(self.expected_updated_documentation_without_home_page_access, delimiter=';', index_col=False)
        res_df = measure_access(self.documented_measurements, self.new_measurement_with_home_page_access_shuffled)
        self.assertTrue(exp_df.equals(res_df))
    

    def test_updating_already_existing_documentation_with_home_page_access_non_shuffled(self):
        exp_df = pandas.read_csv(self.expected_updated_documentation_without_home_page_access, delimiter=';', index_col=False)
        res_df = measure_access(self.documented_measurements, self.new_measurement_with_home_page_access_non_shuffled)
        self.assertTrue(exp_df.equals(res_df))
    ...
    def test_that_access_at_the_same_time_to_games_on_home_page_are_filtered_out_with_only_one_time_stamp(self):
        ...
        exp_df = pandas.DataFrame(expected_aggregation).astype(dtype={'anon-ip': 'object', 'game': 'object', 'n': 'int64'})
