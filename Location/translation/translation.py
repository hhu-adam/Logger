...
def translate(df: pandas.DataFrame, cache: dict[str, str]) -> pandas.DataFrame:
    ...
    assert list(df.columns) == ['anon-ip', 'game', 'n'], "Given dataframe needs to have anon-ip, game and n as attributes!"
    ...
    df.loc[df.head(country_rows).index,'anon-ip'] = df.loc[df.head(country_rows).index,'anon-ip'].apply(lambda x: _ip_to_country(x, cache))
    ...
    if rows - country_rows > 0:
        df.loc[df.tail(unknown_rows).index,'anon-ip'] = df.loc[df.tail(unknown_rows).index,'anon-ip'].apply(lambda: "??")
...
def update_n(old_df: pandas.DataFrame, new_df: pandas.DataFrame) -> pandas.DataFrame:
    assert list(old_df.columns) == ['country', 'game', 'n'], "Given dataframe needs to have country, game and n as attributes!"
    assert list(new_df.columns) == ['country', 'game', 'n'], "Given dataframe needs to have country, game and n as attributes!"

    old_df = old_df.merge(new_df, how="outer", on=['country', 'game'], suffixes=['_documented', '_measured']).fillna(0)
...
def write_translation_log(ip_df: pandas.DataFrame, log_path: str, cache: dict[str, str]) -> None: 
    if not os.path.exists(log_path): # os.stat(log_path).st_size == 0 or not os.path.exists(log_path):
